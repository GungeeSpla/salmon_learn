...
window.myData = {
  bufferAutoKeys: [
    'wave', 'waveNotice', 'goldenIkuraIcon',
...
function updateCanvas() {
  ...
  var delay = window.updateInterval;
  if (myData.centerCount8Delay !== 0) {
    delay = (1000 + window.updateInterval - myData.centerCount8Delay -
      myData.leftTimeSlip) % 1000;
    myData.centerCount8Delay = 0;
  }
  window.updateTimer = setInterval(updateCanvas, delay);
  ...
  if (isStable) {
    ...
    // ノルマについて
    // ノルマの更新はmyData.normaNumが0のときだけでいい
    if (true || myData.normaNum === 0) {
      d.normaNum = checkNormaNum();
      unshiftBuffer('normaNum', d.normaNum);
      // バッファがすべて一致したらノルマを更新する
...
window.updateCenterCount8Rate  = 10;
...
function checkNormaNumOne(x, y) {
  ...
  var h = BW.NORMA_NUM[0].height;
  var imagedata = window.canvas.ctx.getImageData(x, y, w, h);
  var i, i2, w2, m, k, r = -1, g, b, bw, difs = [];
  for (i = 0; i < 10; i++) {
    difs.push({index: i, sum: 0});
  }
  // 数値ごとに…
...
function checkSpecialKind() {
  ...
  var imagedata = window.canvas.ctx.getImageData(x, y, w, h);
  var i, wh = w * h, k, r = -1, g, b, bw, difs = [
    {index: 1, sum: 0},
    {index: 2, sum: 0},
    {index: 3, sum: 0},
    {index: 4, sum: 0}
