		...
		// Wave1以降ならば
			...
			if (this.wave <= 3 && this.isChangingSec) {
				...
				if (typeof soundName == "string" && soundName != "") {
					...
					if (soundName === "spcheck" && settingApp.usingVoice === "amane") {
						soundName += "-" + Math.floor(Math.random() * 4);
					}
					// 再生
		...
		if (this.$wrapper.attr("is_set_event") != "true") {
			...
			this.$settingNorma.each(function(){
				...
				$this.on(app.clickEvent, function (e) {
					...
					if (settingApp.usingVoice === "amane") {
						app.useDefine = app.SMCOUNT_DEFINE_AMANE[app.normaType];
					}
					app.save();
					app.sound.play("switch");
					return false;
