	...
	this.getJqueryObject = function () {
		...
		if (this.$checkSound.attr("is_set_event") != "true") {
			...
			this.$checkFriend.on("change", function () {
				var isChecked = $(this).prop("checked");
				if (isChecked) {
					app.stTimer.timeOffset.enableFriendMode = true;
					...
					app.stTimer.timeOffset.enableFriendMode = false;
					$(".correction_friend").remove();
			...
			this.$checkStOffset.on("change", function () {
				var isChecked = $(this).prop("checked");
				if (isChecked) {
					app.stTimer.enableOffset = true;
					app.$stPlus.removeClass("hidden_button");
					app.$stMinus.removeClass("hidden_button");
					var sign = app.stTimer.offsetMin >= 0 ? "+" : "";
					var str = app.stTimer.offsetMin != -1 ? "(" + sign + app.stTimer.offsetMin + "分)" : "";
					app.$stOffset.text(str);
					if (app.stTimer.offsetMin == 1) {
						app.$stPlus.removeClass("hidden_button");
						app.$stMinus.addClass("hidden_button");
					...
					app.stTimer.enableOffset = false;
					app.$stPlus.addClass("hidden_button");
					app.$stMinus.addClass("hidden_button");
					app.$stOffset.text("");
				}
...
function StTimer () {
	...
	this.listupNextSTT = function (intTime) {
		...
		var offsetMin = this.enableOffset ? this.offsetMin : 0;
		var firstST = this.firstST + offsetMin;
...
function TimeOffset () {
	...
	this.getTime = function () {
		...
		if (this.enableFriendMode) offset -= this.friendOffset;
		return time + offset;
	};
