...
{ 
    ...
    loadedImage.onload = function() {
        ...
        thisRef.texture = gl.createTexture();	 // テクスチャオブジェクトを作成する
        if (!thisRef.texture){ console.error("Failed to generate gl texture name."); return -1; }

        gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, 1);	// imageを上下反転
        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, thisRef.texture);
        ...
        model.setTexture(no, thisRef.texture);// モデルにテクスチャをセット
